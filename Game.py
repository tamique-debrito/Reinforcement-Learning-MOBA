...
STRUCTURE_STATS = lambda: BaseStats(200, 200, 50, 0, TURRET_SIZE, 50, 2.0, TURRENT_ATTACK_RANGE, 300, 300)
...
PLAYER_STATS = lambda: BaseStats(500, 500, 100, 10.0, PLAYER_SIZE, 50, 1.0, PLAYER_ATTACK_RANGE, 200, 100)
...
class Game:
    def __init__(self, team_to_play=TEAM_B, display_elem_tracking_only=False) -> None:
        ...
        self.sim = sim
        self.inputTracker = InputTracker(VISUAL_PADDING, VISUAL_PADDING, VISUAL_SCALE)
        ...
        self.base_A: Turret = sim.unitList[0] # type: ignore
        self.base_B: Turret = sim.unitList[3] # type: ignore

        self.outer_turret_A: Turret = sim.unitList[1] # type: ignore
        self.outer_turret_B: Turret = sim.unitList[2] # type: ignore

        self.outer_turret_A.invulnerable = False # type: ignore
        self.outer_turret_B.invulnerable = False # type: ignore
        ...
        self.display = Display(
            VISUAL_SCALE * MAP_X_DIM + 2 * VISUAL_PADDING, VISUAL_SCALE * MAP_Y_DIM + 2 * VISUAL_PADDING,
            display_elem_tracking_only=display_elem_tracking_only,
            shiftX=VISUAL_PADDING, shiftY=VISUAL_PADDING, scale=VISUAL_SCALE
        ) # TODO: add a "tracking only" property so that the screen doesn't get created
    ...
    def step(self, command=None, alt_command=None, skip_render=False, delay=100):
        ...
        if not self.outer_turret_A.active:
            self.base_A.invulnerable = False
        if not self.outer_turret_B.active:
            self.base_B.invulnerable = False
    ...
    def renderState(self, delay=100, skip_render=False):
        self.display.renderState(self.sim.unitList, delay=delay, skip_render=skip_render)
    ...
    def get_vec_k_units(self, x, y, k):
        return self.sim.get_vec_k_minions(x, y, k), [self.outer_turret_A, self.base_A, self.outer_turret_B, self.base_B]
